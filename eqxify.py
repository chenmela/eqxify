...
'''
Spotify Authorization Code Flow at
...
def home():
    ...
    #Initialize the state with a random value
    session["state"] = get_random_state()

    #Create authorization request params
    AUTH_REQUEST_PARAMS["state"] = session["state"]
    auth_request_args = "&".join(["{}={}".format(key,urllib.quote(value)) 
        for key, value in AUTH_REQUEST_PARAMS.iteritems()])
...
'''
Requests refresh and access tokens to make future API calls
Continues on to logic for adding songs
'''
@app.route("/auth")
def auth():
    ...
    if ("code" not in request.args):
        return "Please authorize access to EQXify. Revisit {}.".format(APP_URI)

    #Check to see that request and repsonse originated in same browser
    if (request.args["state"] != session["state"]):
        return "Request & response states differ. Revisit {}.".format(APP_URI)

    #Get authorization code
    auth_code = request.args["code"]
    
    #Format access token request
    access_params = {
    ...
    if ("access_token" in access_response):
        ...
        #Save access token for future function calls in the same session
        session["access_token"] = access_token
    else:
        #If access token cannot be obtained, request it using refresh token
        return redirect(url_for("refresh"))

    #Continue onto logic for adding songs 
    return redirect(url_for("add_songs"))

'''
Requests a new access token using the refresh token
Continues on to the logic for adding songs
'''
@app.route("/refresh")
def refresh():
    ...
    #Format request for refresh token
    ...
    #Continue on to logic for adding songs
    return redirect(url_for("add_songs"))

'''
Use eqx module to scrape data from 102.7 WEQX online song history log,
Add each of the top hits to the user's Spotify playlist
'''
@app.route("/add_songs")
def add_songs():
    ...
    get_user_request = requests.get(get_user_endpoint, 
    ...
    top_hits = eqx.scrape_data()

    #Print each top hit for demo 
    for hit in top_hits:
        print(hit)
    ...
    for hit in top_hits:
        try:
            #Obtain track URI for each song by making a request
            ...
            query_string = "%20".join(["{}:{}".format(key, value) 
            ...
            search_track_id_endpoint += "&".join(["{}={}".format(key, value) 
                for key, value in search_track_id_params.iteritems()])
            
            search_track_id_request = requests.get(search_track_id_endpoint, 
                    headers=search_track_id_headers)
            
            search_track_id_response = json.loads(search_track_id_request.text)
            track_ids.append(
                    search_track_id_response["tracks"]["items"][0]["uri"])
        except:
            continue

    #Step 10: Check for existence of a playlist called Top 102.7 EQX Hits
    #If it exists, a file with its playlist ID exists. 
    #Otherwise, this file does not exist.
    
    if (not os.path.isfile("playlist_id.txt")):
        #Step 11a: If not present, create a playlist by making a request
        create_playlist_endpoint  = "https://api.spotify.com/"
        create_playlist_endpoint += "v1/users/{}/playlists".format(
                session["user_id"])
        ...
        create_playlist_request = requests.post(create_playlist_endpoint, 
                headers=create_playlist_headers,json=create_playlist_payload)
        ...
        #Step 12: Add songs to playlist just created by making a request
        add_songs_endpoint = "https://api.spotify.com/" 
        add_songs_endpoint += "v1/users/{}/playlists/{}/tracks".format(
                    session["user_id"], session["playlist_id"])
        ...
        add_songs_request = requests.post(add_songs_endpoint, 
                headers=add_songs_headers, json=add_songs_payload)
        add_songs_response = json.loads(add_songs_request.text)
        return "Check out your new playlist!"

    #Step 11b: If present, replace all songs in existing playlist with new ones
    else:
        ...
        replace_tracks_endpoint = "https://api.spotify.com/"
        replace_tracks_endpoint += "v1/users/{}/playlists/{}/tracks".format(
                    session["user_id"], session["playlist_id"])
        ...
        replace_tracks_request = requests.put(replace_tracks_endpoint, 
                headers=replace_tracks_headers, json=replace_songs_payload)
        replace_tracks_response = json.loads(replace_tracks_request.text)
        return "Check out the updates to your playlist!"
